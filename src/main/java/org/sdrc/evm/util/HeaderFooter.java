...
public class HeaderFooter extends PdfPageEventHelper {
	...
	private DashboardExport dashboardExport;

	public HeaderFooter(ServletContext context, DashboardExport dashboardExport) {
...
//		System.out.println("ParentArea name==>"+parentAreaName);
		Image image = null;
		try {
			switch(parentAreaName){
			case "Bihar":
				image = Image.getInstance(context
						.getRealPath("resources\\images\\Evm_header.png"));
				break;
			case "Assam":
				image = Image.getInstance(context
						.getRealPath("resources\\images\\Evm_header_assam.png"));
				break;
			default:
				break;
			}
	...
	public void onEndPage(PdfWriter writer, Document document) {
		...
		try {
			image = Image.getInstance(context
					.getRealPath("resources\\images\\Evm_footer.png"));
			int indentation = 0;
			float scaler = ((document.getPageSize().getWidth() - indentation) / image
					.getWidth()) * 100;
			image.scalePercent(scaler);
		...
		String domainName = "from http://evmindia.org.in";
